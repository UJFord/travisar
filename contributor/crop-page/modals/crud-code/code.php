...
if (isset($_POST['edit']) && $_SESSION['rank'] == 'Curator' || $_SESSION['rank'] == 'Admin') {
    ...
    try {
        ...
        if ($get_category_name === 'Corn') {
            ...
            // abiotic resistance rice
            $query_abioticRes = "UPDATE set abiotic_resistance_rice rice_drought = $1, rice_salinity, = $2, rice_heat = $3, harmful_radiation = $4, rice_abiotic_other = $5, rice_abiotic_other_desc = $6 where abiotic_resistance_rice_id = $7";
            ...
            // disease resistance
            $query_diseaseRes = "UPDATE set disease_resistance bacterial = $1, viral = $2, fungus = $3 where disease_resistance_id = $4";
            $query_run_diseaseRes = pg_query_params($conn, $query_diseaseRes, array($bacterial, $viral, $fungus, $disease_resistance_id));
            if ($query_run_diseaseRes) {
            } else {
                echo "Error: " . pg_last_error($conn);
