            ...
            <form id="form-panel-Edit" name="Form" action="modals/crud-code/abiotic-code.php" autocomplete="off" method="POST" enctype="multipart/form-data" class=" py-3 px-5">
                <div class="modal-body" id="modal-body">
                    <div class="container">
                        <div id="Edit-User">
                            <!-- user id hidden -->
                            <input type="hidden" name="abiotic_idEdit" id="abiotic_idEdit">
                            <!-- abiotic name-->
                            <div class="row mb-3">
                                <!-- abiotic name -->
                                <div class="col">
                ...
                <!-- confirm -->
                <?php require "modals/abiotic-tabs/confirm.php"; ?>
            </form>
        </div>
    </div>
</div>
...
<script>
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $.each(response, function(key, value) {
                        ...
                        // crop_id
                        $('#abiotic_idEdit').val(id);
                        $('#abiotic-NameEdit').val(value['abiotic_name']);
                    });
                },
    ...
    function validateFormEdit() {
        var abiotic_name = document.getElementById('abiotic-NameEdit').value;

        var errors = [];

        // Check if the required fields are not empty
        if (abiotic_name === "" || abiotic_name === null) {
...
<script>
    ...
    document.getElementById('confirmDeleteBtnEdit').addEventListener('click', function() {
        ...
        $.ajax({
            ...
            data: {
                'delete': true,
                'abiotic_resistance_id': document.getElementById('abiotic_idEdit').value
            },
            success: function(response) {
                // Handle the response from the server
                console.log('Abiotic deleted:', response);
