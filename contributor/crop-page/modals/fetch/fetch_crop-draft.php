...
if (isset($_POST['click_draft_btn'])) {
    ...
    if ($query_getCategory_run && pg_num_rows($query_getCategory_run) > 0) {
        $rowDraft = pg_fetch_assoc($query_getCategory_run);
        $category_name = $rowDraft['category_name'];

        // Check if category_name is null or empty
    ...
    if (empty($arrayresult)) {
        // If no category is available or category_name is null or empty, retrieve basic crop data
        ...
        if ($query_run && pg_num_rows($query_run) > 0) {
            $rowDraft = pg_fetch_assoc($query_run);
