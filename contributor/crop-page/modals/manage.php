...
<script>
    document.getElementById('form-panel-view').addEventListener('submit', function(event) {
        ...
        var selectedCategoryManage = document.getElementById('categoryID').value;
        var cornMorphManage = document.getElementById('cornMorph-Manage');
        var riceMorphManage = document.getElementById('riceMorph-Manage');
        var rootCropMorphManage = document.getElementById('root_cropMorph-Manage');

        // Disable inputs based on the selected category
        if (selectedCategoryManage !== '4') {
            disableInputs(cornMorphManage);
        }

        if (selectedCategoryManage !== '1') {
            disableInputs(riceMorphManage);
        }

        if (selectedCategoryManage !== '2') {
            disableInputs(rootCropMorphManage);
        }
