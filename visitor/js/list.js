$(document).ready(function () {
        ...
        // Function to spiderfy clusters
        function spiderfyClusters() {
            markers.eachLayer(function (layer) {
                if (layer instanceof L.Marker && markers.hasLayer(layer)) {
                    var cluster = markers.getVisibleParent(layer);
                    if (cluster instanceof L.MarkerCluster && cluster.getChildCount() > 1) {
                        cluster.spiderfy();
                    }
                }
            });
        ...
        // Automatically spiderfy the clusters on load
        map.whenReady(function() {
            spiderfyClusters();
        });

        // Spiderfy clusters after zoom levels change
        map.on('zoomend', spiderfyClusters);
