<footer class="container-fluid">
    <div class="container">
        <div class="row py-3 text-light">

            <!-- logo -->
            <div class="col-3 d-flex align-items-center">
                <a href="<?=  BASE_URL . '/' . 'visitor/home.php'; ?>"><img class="w-75" src="<?= BASE_URL . '/' . 'img/travis-light.svg'; ?>" alt=""></a>
            </div>

            <!-- about us -->
            <div class="col-3">
                <h4 class="mb-3 footer-head fw-bold">ABOUT</h5>
                    <ul class="list-unstyled">
                        <li>
                            <a href="<?= BASE_URL . '/' . 'visitor/about/sar.php'; ?>" class="link-light link-underline link-underline-opacity-0 link-opacity-50-hover">Sarangani</a>
                        </li>
                        <li>
                            <a href="<?= BASE_URL . '/' . 'visitor/about/collab.php'; ?>" class="link-light link-underline link-underline-opacity-0 link-opacity-50-hover">Collaborators</a>
                        </li>
                        <li>
                            <a href="<?= BASE_URL . '/' . 'visitor/about/travis.php'; ?>" class="link-light link-underline link-underline-opacity-0 link-opacity-50-hover">TRAVIS</a>
                        </li>
                    </ul>
            </div>

            <!-- shortcuts -->
            <div class="col-3">
                <h4 class="mb-3 footer-head fw-bold">LINKS</h5>
                    <ul class="list-unstyled">
                        <li>
                            <a href="<?= BASE_URL . '/' . 'visitor/all.php'; ?>" class="link-light link-underline link-underline-opacity-0 link-opacity-50-hover">Crops</a>
                        </li>
                        <li>
                            <a href="<?= BASE_URL . '/' . 'visitor/all.php?map=open'; ?>" class="link-light link-underline link-underline-opacity-0 link-opacity-50-hover">Map</a>
                        </li>
                        <li>
                            <a href="<?= BASE_URL . '/' . 'login/login.php'; ?>" class="link-light link-underline link-underline-opacity-0 link-opacity-50-hover">Contribute</a>
                        </li>
                    </ul>
            </div>

            <!-- contact -->
            <div class="col-3">
                <h4 class="mb-3 footer-head fw-bold">CONTACT US</h5>
                    <ul class="list-unstyled">
                        <li>
                            <a href="#" class="link-light link-underline link-underline-opacity-0 link-opacity-50-hover"><i class="me-2 fa-solid fa-phone"></i>+63913-234-1283</a>
                        </li>
                        <li>
                            <a href="#" class="link-light link-underline link-underline-opacity-0 link-opacity-50-hover"><i class="me-2 fa-solid fa-envelope"></i>travis@msugensan.edu.ph</a>
                        </li>
                    </ul>
            </div>

        </div>

        <div id="subfoot" class="row text-white rounded-top-5 p-5 py-3">
            <div class=" text-center" style="font-size: 0.9rem;">Mindanao State University - General Santos, Fatima, General Santos City</div>
            <div class=" text-center" style="font-size: 0.9rem;">&copy; Incognito 2024. All Rights Reserved.</div>
        </div>
    </div>
</footer>